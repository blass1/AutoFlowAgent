...
- `reparar-tras-fallo.md` — sub-flow **surgical** unificado de reparación post-fallo. Lo invocan `generar-pom.md` (smoke), `correr-caso.md` y `correr-test-set.md`. Parsea el output de Playwright, identifica el nodo concreto que falló y ofrece reparación dirigida (Auto-Health o pegado a mano sobre ese nodo). Si no puede identificarlo, cae al fallback adivinatorio `actualizar-nodos.md`.
...
| `.autoflow/runs/` | Historial JSON de cada corrida (metadata: status, duración, testIds, `artifactsDir`). Lo lee el dashboard para la pestaña Ejecuciones. **Gitignored**. |
| `.autoflow/alm/runs/` | Sub-carpetas `{run_timestamp}/ResultsALM.json` — **un archivo por corrida** en formato espejo de HP/Micro Focus ALM (`config.legajo` + `tests[]` con `testId`, `testSetId`, `result`, `name`, `duration`, `url_doc` apuntando al PDF físico en `runs/`, `evidence` con el filename). Lo escribe el reporter en cada corrida exitosa; lo consume la integración con ALM (a futuro un .exe lo levanta y sube los resultados al caso correspondiente). **Gitignored**. |
| `runs/` | Carpeta por corrida en la **raíz del repo** con artifacts de Playwright. Cada sub-carpeta tiene formato `DD_MM_YYYY_HH-MM-SS` y la crean los wrappers `run-test.js` / `run-testset.js` antes de invocar Playwright (`AUTOFLOW_RUN_DIR` → `playwright.config.ts` `outputDir`). Si la corrida vino sin wrapper (npx directo, plugin de VSCode), `playwright.config.ts` crea la carpeta él mismo en module-load. **Contenido**: screenshots de fallo + traces + videos + attachments de Playwright; `screens/{testId}/{label}_DD_MM_YYYY_HH_MM_SS.jpg` con las capturas que toma el helper `screen()` en puntos clave del flujo (insertadas auto por `generar-pom.md` en botones de confirmación / pantallas principales, o manualmente vía editar-caso → `📸 Insertar screenshot`); y el **PDF del reporte** `{testId\|slug}.pdf` generado por `lib/pdf-report.js` desde el reporter al cierre — con portada (página 1) que incluye datos del ejecutor (nombre/legajo/equipo/tribu de `user.json`), datos de la ejecución (fecha/duración/IDs), ambiente (canal/URL) y, en mode testset, lista de tests incluidos con status pill. Páginas siguientes con la evidencia (screens embebidos). Background gradient naranja → violeta sobre cards oscuras semitransparentes (paleta Galicia, coherente con el dashboard). Pensado para adjuntarse a ALM. **Gitignored**. |
| `pages/` | Page Objects (los puebla el agente). |
| `tests/` | Specs Playwright (los puebla el agente). |
| `fixtures/index.ts` | Fixtures tipadas (`test.extend`). Sin clase base. Incluye fixture `humanize` + helper `bufferEntreAcciones(page)` para el wait anti-solape entre acciones (centraliza el buffer; respeta env `AUTOFLOW_BUFFER_MS`) + helper `screen(page, label)` que captura JPEG quality 60 viewport-only en `{AUTOFLOW_RUN_DIR}/screens/{testId}/{label}_DD_MM_YYYY_HH_MM_SS.jpg`, esperando `domcontentloaded` y `aria-busy=false` antes de disparar. Si `AUTOFLOW_RUN_DIR` no está seteado retorna `null` silencioso (no rompe). El agente lo inserta **opt-in tras smoke test exitoso** vía `auto-insertar-screens.md` (sub-flow opcional desde `generar-pom.md` paso 11) o manualmente vía `editar-caso.md → 📸 Insertar screenshot`. **El create flow base no inserta screens** — se mantiene liviano y los screens se suman sobre Tests que ya pasan. |
...
- Folders recursivos (con sub-carpetas anidadas): `captures/`, `.autoflow/runs/` (history JSON), `.autoflow/alm/runs/` (ResultsALM por corrida), `runs/` en la raíz (screens + PDFs), `tests/_temp/`, `playwright-report/`, `test-results/`, `blob-report/`.
