...
| `dashboard.js` | Genera `.autoflow/dashboard.html` — vista única navegable del proyecto. **Sidebar**: sección Usuario (perfil del QA, editable vía prompt al agente o descarga de `user.json`) + Test Sets expandibles a Tests. **Cada Page** tiene un color determinístico (hash del nombre) que se aplica en pasos, dots por Test y bordes de cards. **Tab Detalles del Test Set**: Page Objects en grid de 7 cards por fila con métricas (locators, métodos, nodos, confiabilidad promedio, # de Tests que lo usan). **Tab Pasos**: filas coloreadas por Page Object. **Modal de Nodo**: abrir en VSCode + copiar prompt actualización + bifurcar Test desde el nodo. Acepta `--open` para abrir en el browser. |
