...
| `generar-pom.md` | Post-grabación — matchea pages existentes, agrupa pasos, genera POMs, sidecars, nodos, traza y spec. **Limpieza pre-agrupado** (paso 2.5): muestra todos los pasos parseados con multi-select para que el QA borre los no deseados; los nodos restantes se renumeran. **Listado colapsado**: las pages ya agrupadas se muestran en una línea (`✅ X (pasos N–M)`), solo el bloque "Nuevo" va completo. **Regrafos** una sola vez al final (paso 9.5), no per agrupación. **Colisión de nombres** (paso 5.5): delega en `pom-colision.md`. **Modo añadir pasos** (paso 0): delega en `pom-append-grabado.md`. **Matcheo de prefijo sigue `deprecated → reemplazadoPor`** (paso 3): si el grabador captura el selector viejo de un Nodo ya saneado por Auto-Health Node, el matcher lo resuelve al id live y reusa la Page sin obligar al QA a regroupar. |
