...
| Quiero borrar todo y empezar de cero | `node .autoflow/clearSession.js` (pide confirmación con `SI`). Borra todo lo generado por el agente (recordings, pages, tests, fingerprints, testsets, nodos.json, grafos, dashboard, captures, runs JSON, auth, ALM exports/originalTests/imports, data/data-*.ts, y recursivamente `runs/` raíz con screens+PDFs, `tests/_temp/`, `playwright-report/`, `test-results/`). **Preserva** node_modules, `user.json` (tu identidad de QA — no rehacés onboarding), `alm/integrations/*.exe` (binarios propietarios), seeds de `data/`, prompts, scripts, conventions, fixtures, configs, utils, `.gitkeep`. |
