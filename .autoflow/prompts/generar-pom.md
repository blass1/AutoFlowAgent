...
2. Generá el archivo del PO siguiendo las reglas. Naming:
   ...
   **El JSDoc de la clase queda en una línea** (descripción corta de la pantalla o del componente, en español, sin listar acciones). Reglas críticas para que el test pase en primera corrida:
   ...
   - **Screenshots automáticos**: durante este paso **NO insertes** llamadas a `screen()` ni nodos `capturar-screen`. La inserción de screens se hizo opcional y se ofrece **post-smoke test exitoso** (ver paso 11.5). Mantener `generar-pom.md` enfocado solo en el modelado del flujo reduce la carga cognitiva del agente y evita que el create rompa si la heurística de screens falla. Si el QA quiere screens manuales puntuales, los agrega después vía `editar-caso.md → 📸 Insertar screenshot`.
...
- `🚦 Smoke test ahora (recomendado)` → corré el Test recién generado headless para validar que los selectores capturados realmente funcionan en el sitio. Usá `runCommands`:
  ...
  Si el smoke pasa → mostrá `✅ El Test pasó. Listo para commitear.` y abrí single-select `"¿Algo más?"`:
    - `📸 Agregar screenshots automáticos para evidencia` → cargá `.autoflow/prompts/auto-insertar-screens.md` con contexto `{ specPath, slug, numero }`. Ese sub-flow analiza POMs + spec, propone puntos de captura, confirma con el QA y aplica los cambios. Cuando vuelve, **reabrí esta misma pregunta** (sin re-correr el smoke).
    - `▶️ Correrlo headed para verlo en pantalla` → headed run.
    - `🏠 Volver al menú`.
  Si el smoke **falla** → seguí al **bloque 11.1** (Auto-reparar tras smoke fallido). Los screens no se ofrecen acá — primero hay que tener el Test pasando.
